...
var WorkboxSW = (function () {
    ...
    function parse(a, b) {
        for (var c, d = [], e = 0, f = 0, g = '', h = b && b.delimiter || '/'; null != (c = PATH_REGEXP.exec(a));) {
            var i = c[0], j = c[1], k = c.index;
            if (g += a.slice(f, k), f = k + i.length, j) {
                g += j[1];
                continue
            }
            var l = a[f], m = c[2], n = c[3], o = c[4], p = c[5], q = c[6], r = c[7];
            g && (d.push(g), g = '');
            var s = c[2] || h, t = o || p;
            d.push({
                name: n || e++,
        ...
        return function (c, d) {
            for (var e, f = '', g = c || {}, h = d || {}, k = h.pretty ? encodeURIComponentPretty : encodeURIComponent, l = 0; l < a.length; l++) {
                ...
                if (index$1(m)) {
                    ...
                    if (0 === m.length) if (e.optional) continue; else throw new TypeError('Expected "' + e.name + '" to not be empty');
    ...
    function tokensToRegExp(a, b, c) {
        ...
        for (var d, e = c.strict, f = !1 !== c.end, g = '', h = 0; h < a.length; h++) if (d = a[h], 'string' == typeof d) g += escapeString(d); else {
            var i = escapeString(d.prefix), j = '(?:' + d.pattern + ')';
            b.push(d), d.repeat && (j += '(?:' + i + j + ')*'), j = d.optional ? d.partial ? i + '(' + j + ')?' : '(?:' + i + '(' + j + '))?' : i + '(' + j + ')', g += j;
        }
        var k = escapeString(c.delimiter || '/'), l = g.slice(-k.length) === k;
        ...
        constructor({path: a, handler: b, method: c}) {
            ...
            super({
                match: ({url: b}) => {
                    ...
                    const c = a.startsWith('/') ? b.pathname : b.href, f = c.match(e);
                    if (!f) return null;
                    const g = {};
                ...
                match: ({event: d, url: e}) => {
                    ...
                    if ('navigate' === d.request.mode) {
                        ...
                        a.some((a) => a.test(d)) ? b.some((a) => a.test(d)) ? f = `The navigation route is not being used, since the ` + `request URL matches both the whitelist and blacklist.` : (f = `The navigation route is being used.`, g = !0) : f = `The navigation route is not being used, since the ` + `URL being navigated to doesn't match the whitelist.`, logHelper.debug({
                            that: this,
                            message: f,
                            data: {"request-url": e.href, whitelist: a, blacklist: b, handler: c}
                        });
                    }
                    return g
    ...
    class Router$2 {
        ...
        handleRequest({event: a}) {
            ...
            let {handler: c, params: d} = this._findHandlerAndParams({event: a, url: b});
        ...
        unregisterRoutes({routes: a} = {}) {
            ...
            routes registered.`, data: {route: b}
                ...
                -1 < a ? this._routes.get(b.method).splice(a, 1) : logHelper.error({
    ...
    class Router$$1 extends Router$2 {
        ...
        registerRoute(a, b, c = 'GET') {
            ...
            if ('string' == typeof a) {
                ...
                const e = a.startsWith('http') ? new URL(a, location).pathname : a, f = e.match(/[*:?+]/);
                f && logHelper.warn({
                    ...
                    data: {"Path String": a, "Wildcard Character": f[0], "Learn More": 'https://goo.gl/xZMKEV'}
    ...
    var cleanResponseCopy = (({response: a}) => {
        ...
        return c.then((a) => new Response(a, {headers: b.headers, status: b.status, statusText: b.statusText}))
    });
    ...
    class RequestWrapper {
        constructor({cacheName: a, cacheId: b, plugins: c, fetchOptions: d, matchOptions: e} = {}) {
            ...
            a ? (isType({cacheName: a}, 'string'), this.cacheName = a, b && (this.cacheName = `${b}-${this.cacheName}`)) : this.cacheName = getDefaultCacheName({cacheId: b}), d && (isType({fetchOptions: d}, 'object'), this.fetchOptions = d), e && (isType({matchOptions: e}, 'object'), this.matchOptions = e), this.plugins = new Map, c && (isArrayOfType({plugins: c}, 'object'), c.forEach((a) => {
                for (let b of pluginCallbacks) if ('function' == typeof a[b]) {
                    if (!this.plugins.has(b)) this.plugins.set(b, []); else if ('cacheWillUpdate' === b) throw ErrorFactory$4.createError('multiple-cache-will-update-plugins'); else if ('cachedResponseWillBeUsed' === b) throw ErrorFactory$4.createError('multiple-cached-response-will-be-used-plugins');
        ...
        fetch({request: a}) {
            ...
            return asyncToGenerator(function* () {
                ...
                if (b.plugins.has('requestWillFetch')) for (let c of b.plugins.get('requestWillFetch')) {
                ...
                try {
                    ...
                    if (b.plugins.has('fetchDidFail')) for (let a of b.plugins.get('fetchDidFail')) yield a.fetchDidFail({request: c.clone()});
                    throw a
                }
            })()
        }

        fetchAndCache({request: a, waitOnCache: b, cacheKey: c, cacheResponsePlugin: d, cleanRedirects: e}) {
            var f = this;
            return asyncToGenerator(function* () {
                atLeastOne({request: a});
                let g;
                ...
                if (j) {
                    const b = e && h.redirected ? yield cleanResponseCopy({response: h}) : h.clone();
                    g = f.getCache().then((() => {
                        var d = asyncToGenerator(function* (d) {
                            let e;
                            const g = c || a;
                            if ('opaque' !== h.type && f.plugins.has('cacheDidUpdate') && (e = yield f.match({request: g})), yield d.put(g, b), f.plugins.has('cacheDidUpdate')) for (let a of f.plugins.get('cacheDidUpdate')) yield a.cacheDidUpdate({
    ...
    class CacheFirst extends Handler {
        handle({event: a} = {}) {
            ...
            return asyncToGenerator(function* () {
                ...
                return c || (yield b.requestWrapper.fetchAndCache({request: a.request, waitOnCache: b.waitOnCache}))
            })()
        }
    }
    ...
    class CacheOnly extends Handler {
        handle({event: a} = {}) {
            var b = this;
            return asyncToGenerator(function* () {
                return isInstance({event: a}, FetchEvent), yield b.requestWrapper.match({request: a.request})
            })()
        }
    }
    ...
    class NetworkOnly extends Handler {
        handle({event: a} = {}) {
            var b = this;
            return asyncToGenerator(function* () {
                return isInstance({event: a}, FetchEvent), yield b.requestWrapper.fetch({request: a.request})
            })()
        }
    }
    ...
    var idb = createCommonjsModule(function (a) {
        ...
        (function () {
            ...
            f(j, '_index', ['name', 'keyPath', 'multiEntry', 'unique']), g(j, '_index', IDBIndex, ['get', 'getKey', 'getAll', 'getAllKeys', 'count']), i(j, '_index', IDBIndex, ['openCursor', 'openKeyCursor']), f(k, '_cursor', ['direction', 'key', 'primaryKey', 'value']), g(k, '_cursor', IDBCursor, ['update', 'delete']), ['advance', 'continue', 'continuePrimaryKey'].forEach(function (a) {
                ...
                [l, j].forEach(function (c) {
                    c.prototype[a.replace('open', 'iterate')] = function () {
                        var c = b(arguments), d = c[c.length - 1], e = this._store || this._index,
            ...
            var p = {
                open: function (a, b, c) {
                    var e = d(indexedDB, 'open', [a, b]), f = e.request;
    ...
    class CacheExpiration {
            ...
            return asyncToGenerator(function* () {
                ...
                if (!b._dbs.has(c)) {
                    const d = yield idb.open(c, idbVersion, function (b) {
                        ...
                        c.createIndex(timestampPropertyName, timestampPropertyName, {unique: !1});
                    });
                    b._dbs.set(c, d);
                }
                return b._dbs.get(c)
        ...
        isResponseFresh({cacheName: a, cachedResponse: b, now: c} = {}) {
            if (b && this.maxAgeSeconds) {
                ...
                if (d) {
                    ...
                    const a = new Date(d), b = a.getTime();
        ...
        updateTimestamp({cacheName: a, url: b, now: c} = {}) {
            ...
            return asyncToGenerator(function* () {
                ...
                const f = yield d.getDB({cacheName: a}), g = f.transaction(a, 'readwrite');
                g.objectStore(a).put({[timestampPropertyName]: c, [urlPropertyName]: e.href}), yield g.complete;
            })()
        }
        ...
        expireEntries({cacheName: a, now: b} = {}) {
            ...
            return asyncToGenerator(function* () {
                ...
                const d = c.maxAgeSeconds ? yield c.findOldEntries({cacheName: a, now: b}) : [],
                    e = c.maxEntries ? yield c.findExtraEntries({cacheName: a}) : [], f = [...new Set(d.concat(e))];
                if (yield c.deleteFromCacheAndIDB({cacheName: a, urls: f}), 0 < f.length && logHelper.debug({
                    that: c,
                    ...
                    return c._timestampForNextRun = null, c.expireEntries({cacheName: a, now: b})
                }
            })()
        }
        ...
        findOldEntries({cacheName: a, now: b} = {}) {
            ...
            return asyncToGenerator(function* () {
                ...
                const d = b - 1e3 * c.maxAgeSeconds, e = [], f = yield c.getDB({cacheName: a}),
        ...
        findExtraEntries({cacheName: a} = {}) {
            ...
            return asyncToGenerator(function* () {
                isType({cacheName: a}, 'string');
                const c = [], d = yield b.getDB({cacheName: a});
                let e = d.transaction(a, 'readonly'), f = e.objectStore(a), g = f.index(timestampPropertyName);
        ...
        deleteFromCacheAndIDB({cacheName: a, urls: b} = {}) {
            ...
            return asyncToGenerator(function* () {
                if (isType({cacheName: a}, 'string'), isArrayOfType({urls: b}, 'string'), 0 < b.length) {
                    const d = yield c.getCache({cacheName: a}), e = yield c.getDB({cacheName: a});
                    for (let c of b) {
                        yield d.delete(c);
                        const b = e.transaction(a, 'readwrite'), f = b.objectStore(a);
    ...
    class CacheExpirationPlugin extends CacheExpiration {
        ...
        cacheDidUpdate({cacheName: a, newResponse: b, url: c, now: d} = {}) {
            var e = this;
            return asyncToGenerator(function* () {
                isType({cacheName: a}, 'string'), isInstance({newResponse: b}, Response), 'undefined' == typeof d && (d = Date.now()), yield e.updateTimestamp({
                    cacheName: a,
                    url: c,
                    now: d
    ...
    function broadcastUpdate({channel: a, cacheName: b, url: c, source: d} = {}) {
        isInstance({channel: a}, BroadcastChannel), isType({cacheName: b}, 'string'), isType({source: d}, 'string'), isType({url: c}, 'string'), a.postMessage({
    ...
    function responsesAreSame({first: a, second: b, headersToCheck: c} = {}) {
        ...
        because none of the headers that would be checked are present.`,
            data: {"First Response": a, "Second Response": b, "Headers To Check": JSON.stringify(c)}
        }), !0)
    }
    ...
    class BroadcastCacheUpdate {
        ...
        notifyIfUpdated({first: a, second: b, cacheName: c, url: d}) {
            isType({cacheName: c}, 'string'), responsesAreSame({
                first: a,
                second: b,
                headersToCheck: this.headersToCheck
            }) || broadcastUpdate({cacheName: c, url: d, channel: this.channel, source: this.source});
        }
    }
    ...
    class BroadcastCacheUpdatePlugin extends BroadcastCacheUpdate {
        cacheDidUpdate({cacheName: a, oldResponse: b, newResponse: c, url: d}) {
            isType({cacheName: a}, 'string'), isInstance({newResponse: c}, Response), b && this.notifyIfUpdated({
                cacheName: a,
                first: b,
                second: c,
                url: d
    ...
    class BaseCacheManager {
        constructor({cacheName: a, cacheId: b, plugins: c} = {}) {
            if (b && ('string' != typeof b || 0 === b.length)) throw new WorkboxError('bad-cache-id', {cacheId: b});
        ...
        _addEntryToInstallList(a) {
            const b = a.entryID, c = this._entriesToCache.get(a.entryID);
        ...
        _cacheEntry(a) {
            var b = this;
            return asyncToGenerator(function* () {
                const c = yield b._isAlreadyCached(a), d = {url: a.request.url, revision: a.revision, wasUpdated: !c};
                if (c) return d;
                try {
                    ...
                    throw new WorkboxError('request-not-cached', {url: a.request.url, error: b})
                }
            })()
        }
    ...
    class IDBHelper {
        ...
        put(a, b) {
            return this._getDb().then((c) => {
                const d = c.transaction(this._storeName, 'readwrite'), e = d.objectStore(this._storeName);
        ...
        delete(a) {
            return this._getDb().then((b) => {
                const c = b.transaction(this._storeName, 'readwrite'), d = c.objectStore(this._storeName);
    ...
    class BaseCacheEntry {
        ...
        getNetworkRequest() {
            ...
            const b = {};
            if (!0 === this.cacheBust) if ('cache' in Request.prototype) b.cache = 'reload'; else {
    ...
    class StringCacheEntry extends BaseCacheEntry {
        constructor(a) {
            if (isType({url: a}, 'string'), 0 === a.length) throw new WorkboxError('invalid-string-entry', {url: a});
    ...
    class ObjectCacheEntry extends BaseCacheEntry {
        constructor({entryID: a, revision: b, url: c, cacheBust: d}) {
            if ('undefined' != typeof b && (isType({revision: b}, 'string'), 0 === b.length)) throw new WorkboxError('invalid-object-entry', {
                problemParam: 'revision',
                problemValue: b
            });
            if ('undefined' == typeof d && (d = !!b), isType({cacheBust: d}, 'boolean'), isType({url: c}, 'string'), 0 === c.length) throw new WorkboxError('invalid-object-entry', {
                problemParam: 'url',
                problemValue: c
            });
            if ('undefined' == typeof a) a = new URL(c, location).toString(); else if (0 === a.length) throw new WorkboxError('invalid-object-entry', {
                problemParam: 'entryID',
                problemValue: a
            });
            super({entryID: a, revision: b || c, request: new Request(c), cacheBust: d});
        }
    }
    ...
    class RevisionedCacheManager extends BaseCacheManager {
        ...
        _parseEntry(a) {
            if (null === a) throw new WorkboxError('unexpected-precache-entry', {input: a});
            let b;
            switch (typeof a) {
                case'string':
        ...
        install() {
            return super.install().then((a) => {
                const b = [], c = [];
                a.forEach((a) => {
                    a.wasUpdated ? b.push({url: a.url, revision: a.revision}) : c.push({
                        url: a.url,
                        revision: a.revision
                    });
                });
    ...
    class WorkboxSW$1 {
        constructor({cacheId: a, skipWaiting: b, clientsClaim: c, handleFetch: d = !0, directoryIndex: e = 'index.html', precacheChannelName: f = 'precache-updates', ignoreUrlParametersMatching: g = [/^utm_/]} = {}) {
          ...
          to the smaller, optimized production build of Workbox.`)), a && ('string' != typeof a || 0 === a.length)) throw ErrorFactory.createError('bad-cache-id');
            ...
            if ('undefined' != typeof e) if (!1 === e || null === e) e = !1; else if ('string' != typeof e || 0 === e.length) throw ErrorFactory.createError('bad-directory-index');
            const h = [];
            f && h.push(new BroadcastCacheUpdatePlugin({
                channelName: f,
                source: registration && registration.scope ? registration.scope : location
            })), this._runtimeCacheName = getDefaultCacheName({cacheId: a}), this._revisionedCacheManager = new RevisionedCacheManager({
                cacheId: a,
                plugins: h
            }), this._strategies = new Strategies({cacheId: a}), this._precacheRouter = new Router$$1(this._revisionedCacheManager.getCacheName()), this._router = new Router$$1(this._revisionedCacheManager.getCacheName()), d && (this._precacheRouter.addFetchListener(), this._router.addFetchListener()), this._registerInstallActivateEvents(b, c), this._registerDefaultRoutes(g, e);
        }
        ...
        _registerInstallActivateEvents(a, b) {
            self.addEventListener('install', (b) => {
                ...
                0 < c.length && logHelper.debug({
                    that: this, message: `The precached URLs will automatically be served using a
            cache-first strategy.`, data: {"Precached URLs": JSON.stringify(c)}
        ...
        _getCacheMatchPlugin(a, b) {
            var c = this;
            const d = (() => {
                var d = asyncToGenerator(function* ({request: d, cache: e, cachedResponse: f, matchOptions: g}) {
                    if (f) return f;
        ...
        _removeIgnoreUrlParams(a, b) {
            const c = new URL(a), d = c.search.slice(1), e = d.split('&'), f = e.map((a) => a.split('=')),
                g = f.filter((a) => b.every((b) => !b.test(a[0]))), h = g.map((a) => a.join('='));
